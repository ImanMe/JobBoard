...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'Jobs') AND [c].[name] = N'MinimumExperience');
IF @var5 IS NOT NULL EXEC(N'ALTER TABLE [Jobs] DROP CONSTRAINT [' + @var5 + '];');
ALTER TABLE [Jobs] ALTER COLUMN [MinimumExperience] smallint NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'Jobs') AND [c].[name] = N'MaximumExperience');
IF @var6 IS NOT NULL EXEC(N'ALTER TABLE [Jobs] DROP CONSTRAINT [' + @var6 + '];');
ALTER TABLE [Jobs] ALTER COLUMN [MaximumExperience] smallint NULL;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20170826042247_OtherFixes', N'2.0.0-rtm-26452');
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'Jobs') AND [c].[name] = N'Salary');
IF @var7 IS NOT NULL EXEC(N'ALTER TABLE [Jobs] DROP CONSTRAINT [' + @var7 + '];');
ALTER TABLE [Jobs] ALTER COLUMN [Salary] decimal(18, 2) NULL;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20170826042956_SalaryOptional', N'2.0.0-rtm-26452');
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'Jobs') AND [c].[name] = N'ExpirationDate');
IF @var8 IS NOT NULL EXEC(N'ALTER TABLE [Jobs] DROP CONSTRAINT [' + @var8 + '];');
ALTER TABLE [Jobs] ALTER COLUMN [ExpirationDate] date NOT NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'Jobs') AND [c].[name] = N'EditedDate');
IF @var9 IS NOT NULL EXEC(N'ALTER TABLE [Jobs] DROP CONSTRAINT [' + @var9 + '];');
ALTER TABLE [Jobs] ALTER COLUMN [EditedDate] date NOT NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'Jobs') AND [c].[name] = N'Currency');
IF @var10 IS NOT NULL EXEC(N'ALTER TABLE [Jobs] DROP CONSTRAINT [' + @var10 + '];');
ALTER TABLE [Jobs] ALTER COLUMN [Currency] nvarchar(120) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'Jobs') AND [c].[name] = N'CreatedDate');
IF @var11 IS NOT NULL EXEC(N'ALTER TABLE [Jobs] DROP CONSTRAINT [' + @var11 + '];');
ALTER TABLE [Jobs] ALTER COLUMN [CreatedDate] date NOT NULL;
...
IF @var12 IS NOT NULL EXEC(N'ALTER TABLE [Jobs] DROP CONSTRAINT [' + @var12 + '];');
ALTER TABLE [Jobs] ALTER COLUMN [CreatedBy] date NOT NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'Jobs') AND [c].[name] = N'ActivationDate');
IF @var13 IS NOT NULL EXEC(N'ALTER TABLE [Jobs] DROP CONSTRAINT [' + @var13 + '];');
ALTER TABLE [Jobs] ALTER COLUMN [ActivationDate] date NOT NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'Applicants') AND [c].[name] = N'Date');
IF @var14 IS NOT NULL EXEC(N'ALTER TABLE [Applicants] DROP CONSTRAINT [' + @var14 + '];');
ALTER TABLE [Applicants] ALTER COLUMN [Date] date NOT NULL;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20170826045224_DateTimeToDate', N'2.0.0-rtm-26452');
